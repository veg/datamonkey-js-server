...
ExecuteAFile			("../Shared/CheckExtraTime.bf");
...
maxTimeMod = checkExtraTimeStatus (fileSpec);
if (Type (maxTimeMod) == "Number") {
    maxTimeAllowed = maxTimeMod * 3600;
}
...
baseParams 		   		= Columns(lfInfo["Global Independent"])+freqParamCount;
perPartParameterCount	= Columns(lfInfo["Local Independent"]);
...
fixGlobalParameters ("lf_base");
//fprintf ("lf_globals.dump", CLEAR_FILE, _lfInfo);


Export (base_lf, lf_base);
//fprintf ("lf_base.dump", CLEAR_FILE, base_lf);
...
//fprintf ("mpi_export.dump", CLEAR_FILE, _mpiPrefixString);
...
if (Rows(bestIndividualOverall))
{
	...
	{
	    fprintf (progressFilePath, "\n<DIV class = 'RepClassSM'>Working on segment ", pccounter+1, "</DIV>");
		for (pc2 = 0; pc2 <  readPCount; pc2 += 1)
		{
			if (Abs(pc2-pccounter) <= 1)
			{
	            ...
	            fprintf (progressFilePath, "\n<DIV class = 'RepClassSM'>Fitting tree ", pc2+1, " to segment ", pccounter+1, "</DIV>");
				Optimize			(aRes, aLF);	
		...
		for (pc2 = 0; pc2 <  readPCount; pc2 += 1)
		{
			if (Abs(pc2-pccounter) == 1)
			{
	            ...
	            fprintf (progressFilePath, "\n<DIV class = 'RepClassSM'>Testing tree ", pc2+1, " on segment ", pccounter+1, "</DIV>");
...
function	reportImprovementHTML (_IC)
{
	if (lastImprovedBPC)
	{
		return "<DIV class = 'RepClassSM'><b>GARD found evidence of "+lastImprovedBPC+" breakpoints</b><p>"+spoolAICTable()+
