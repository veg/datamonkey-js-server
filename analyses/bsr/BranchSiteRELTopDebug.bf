...
ExecuteAFile ("phase0.out");
fscanf 		 ("phase0.mx", "NMatrix", res_base);
...
while (k < totalBranchCount)
{
	for							  (k = 0; k < totalBranchCount; k = k+1)
	{
		...
		fprintf (progressFilePath, CLEAR_FILE,  "<DIV class = 'RepClassSM'><DL><DT class = 'DTH'>Branch: ", ref, 
						 ...
						 "</DT><DT class = 'DT1'>Class 3: omega = ", thisOmega3, " weight = ", wt3 , "</DT></DL>"
						 );
...
function SendAJob (bID)
{
	...
	fprintf (stdout, "<br>Testing for selection at this branch. Sent to MPI node ", nodeID+1 ,"</DIV>", previousFile);
...
function ReceiveJobs (rereadPrevious)
{
	...
	{
		fprintf 					  (stdout, "\n<DIV class = 'RepClassSM'>[PHASE 2/REPEAT] Detected a convergence problem. <br> The log likelihood from the null model for branch ",
	...
	{				
		...
		fprintf (stdout,"\n\n<DIV class = 'RepClassSM'>UNCORRECTED p-value for branch ", bNames[branchID] ," = ", pValueByBranch[branchID][8],".<br>Model Log L = ", three_LF_MLES[1][0],"</DIV>", p2p);
	}
