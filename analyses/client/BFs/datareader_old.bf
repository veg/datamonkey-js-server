...
if (ds.sites < 3)
{
	dummy = ErrorOut("This doesn't seem to be a valid alignment file. Please refer to the list of file <a href='http://www.hyphy.org/docs/shared/dataformats.html' target = '_blank'>formats</a> that HyPhy supports."+
...
if (genCodeID != (-2)) /* not an AA alignment */
{
	if (Rows(freqs) != 4)
	{
		dummy = ErrorOut("This doesn't seem to be a nucelotide alignment. It had <div class = 'RepClassSM'>"+Rows(freqs)+"</div>character states, whereas we expected 4. Perhaps you uploaded an amino-acid alignment by mistake?");
		return 0;
	}
}
else
{
	if (Rows(freqs) != 20)
	{
		dummy = ErrorOut("This doesn't seem to be a protein alignment. It had <div class = 'RepClassSM'>"+Rows(freqs)+"</div>character states, whereas we expected 20. Perhaps you uploaded a nucleotide alignment by mistake?");
		return 0;
	}
}
...
{
	dummy = ErrorOut ("The number of nucleotide columns in the data set must be divisible by 3 - had "+ds.sites+
...
if (genCodeID >= 0)
{
	...
	{
		dummy = ErrorOut("Only stop codons were found in your alignment.");
		return 0;
	}
}
else
...
if (genCodeID >= 0 && filteredData.sites*3 < ds.sites)
{
	...
	reportString*512;
	reportString*"<table border = '1'><tr class='HeaderClassSM'><td>Sequence Name</td><td>Position (in nucleotides)</td><td>Codon found</td></tr>";
...
if (filteredData.species > maxSLACSize || filteredData.sites > maxDMSites)
{
...
{
	fprintf (stdout, "<DIV class = 'ErrorTagSM'><b>",dupSeqCount,"</b> duplicate sequences found and removed. You can  <a href='",BASE_CGI_URL_STRING,"showdata.pl?",filePath,"'>look</a> at the reduced alignment in NEXUS format for reference.<DL>",DuplicateSequenceWarning,"</DL></DIV>");
}

if (Abs(renameSequenceWarning))
{
	fprintf (stdout, "<DIV class = 'ErrorTagSM'><b>",renames,"</b> sequences were renamed to conform to HyPhy standards. You can  <a href='",BASE_CGI_URL_STRING,"showdata.pl?",filePath,"'>look</a> at the renamed alignment in NEXUS format for reference.<DL>",renameSequenceWarning,"</DL></DIV>");
}
...
if (padWarning)
{
	fprintf (stdout, "<DIV class='ErrorTagSM'>It appears that some of the sequences were of unequal length and were padded by HyPhy. This could be because unaligned sequences were uploaded or non-standard characters were used to mark gaps ('-' and '?' are allowed; but '~' (BioEdit) and '_' for example, are not)."+
					 "<b>Always use</b> the standard IUPAC-IUB <a href='http://www.hyphy.org/docs/shared/nuctable.html'>character table</a> to prepare the alignment for DataMonkey.org.</DIV>");
}
...
if (dupSeqCount || renames || internalRenames)
{
