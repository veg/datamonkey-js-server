...
#include 		"../Shared/char_colors.def";
ExecuteAFile 	("../Shared/GrabBag.bf");
ExecuteAFile 	("../Shared/PostScriptNew.bf");


SetDialogPrompt ( "Select an amino acid file to process:" );
DataSet ds		= ReadDataFile( PROMPT_FOR_FILE );
_baseName		= LAST_FILE_PATH;
DataSetFilter filteredData = CreateFilter ( ds, 1 );

_font_size  = 14;
_char_space_mult = 1 + 0.1*((Log (filteredData.sites)/Log(2) + 1.5)$1);
_char_space = (_font_size*_char_space_mult);
_page_w	    = 612;
_page_h     = 792;
_atom		= 5;

outFile = _baseName + ".ps";
fprintf (outFile,CLEAR_FILE,_HYPSPageHeader (_page_w,_page_h,"Character Plot for "+_baseName),
...
_unit    = {1,_ccount}["1"];
GetDataInfo (_dupInfo, filteredData);
_result_cache = {};
_maxD		  = 0;
...
for (_idx = 0; _idx < filteredData.sites; _idx = _idx + 1)
{
	_siteInfo = {_ccount, 2};
	_cCounter = {_ccount, 1};
	for (_sidx = 0; _sidx < filteredData.species; _sidx = _sidx + 1)
	{
		GetDataInfo (_thisChar, filteredData, _sidx, _dupInfo[_idx]);
		/* don't count gaps */
...
for (_idx = 0; _idx < filteredData.sites; _idx = _idx + 1)
{
	...
	if ((_idx + 1)%_char_per_line==0 || _idx == filteredData.sites-1)
	{
		_current_y = _current_y + _font_size;
		if (_idx == filteredData.sites-1)
		{
			if (filteredData.sites % _char_per_line == 0)
			{
				_startx = _char_per_line;
			}
			else
			{
				_startx = filteredData.sites%_char_per_line$_atom*_atom;
			}
			_idx2 = filteredData.sites-filteredData.sites%_char_per_line;
		}
		else
		{
