...
if ( Abs ( sitesWithCoverage ) > 0 ) {
	...
	for (rateCount = 2; rateCount < 10; rateCount += 1)
	{
		...
		ExecuteCommands ("global `thisRate` = 0.1; `thisRate` :< 0.999999; `thisRate` :> 0.000001;");
...
{
	LL = 0;
	logLByRateClass = {Rows(rates)$2,1};
	for (k = 0; k < Rows (counts); k += 1)
	{
		maxV = -1e100;
		for (part = 0; part < Rows(rates); part += 2)
		{
			logLByRateClass[part$2] = binomialP (rates[part], counts[k][0], counts[k][1]);
			maxV = Max(maxV,logLByRateClass[part$2]);
		}
		l1 = (logLByRateClass+(-maxV))["Exp(_MATRIX_ELEMENT_VALUE_)*rates[_MATRIX_ELEMENT_ROW_*2+1]"];
		LL += Log (+l1) + maxV;
	}
