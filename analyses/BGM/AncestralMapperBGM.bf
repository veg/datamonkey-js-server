...
function _buildAncestralCache (_lfID, _lfComponentID, _is_sampled)
{
...
/* 2; construct a temporary likelihood function with 
   the tree and the filter; and an AVL representation of the tree */
	...
	if (_is_sampled)
	{
		DataSet _bac_ancDS = SampleAncestors (_bac_tree_LF);
	}
	else
	{
	...
	ExecuteCommands ("GetString (_bacSequenceNames,"+ _bac_filterID +",-1);");
...
/*
	Return a binary vector (matrix) indicating substitution events by site and branch.
*/
	function _mapMutations (_ancID, _siteID, _reportNS)
	{
		if (Abs (_ancestralRecoveryCache[_ancID]))
		{
			if (_siteID >= 0 && _siteID < Columns ((_ancestralRecoveryCache[_ancID])["MATRIX"]))
			{
				...
				_bacSiteC 		    = (_ancestralRecoveryCache[_ancID])["CHARS"];
				_bacSiteDim 		= Columns (_bacSiteC);
				_bacCounter 		= Rows (_thisColumn)-1;	/* skip root? -afyp */
				...
				for (_bacTreeIterator = 0; _bacTreeIterator < _bacCounter; _bacTreeIterator = _bacTreeIterator + 1)
				{
					...
					/* derived state not equal to parental state, ignore ambiguous character states */
					if (_myState >= 0 && _pState >= 0 && _myState != _pState)
					{
						if (_bacSiteDim > 20)
						{
							res[_bacTreeIterator] = 0;
							
							if (_GC_[_myState] != _GC_[_pState] && _reportNS)
							{
								res[_bacTreeIterator] = 1;
							}
