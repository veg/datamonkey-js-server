...
STATUS_FILE=$sfn
PROGRESS_FILE=$pfn
RESULTS_FN=$fn.PRIME.json
GENETIC_CODE=$genetic_code
BRANCHES=${branches:-"All"}
PROPERTY_SET=${property_set:-"5PROP"}
PVALUE=${pvalue:-"0.1"}
...
PRIME=$HYPHY_PATH/TemplateBatchFiles/SelectionAnalyses/PRIME.bf
...
echo "PROPERTY_SET: '$PROPERTY_SET'"
echo "PVALUE: '$PVALUE'"
...
if [ -n "$SLURM_JOB_ID" ]; then
  ...
  if [ -f "$HYPHY_NON_MPI" ]; then
    ...
    export TOLERATE_NUMERICAL_ERRORS=1
    echo "srun --mpi=$MPI_TYPE -n $PROCS $HYPHY LIBPATH=$HYPHY_PATH $PRIME --alignment $FN --tree $TREE_FN --code $GENETIC_CODE --branches $BRANCHES --property-set $PROPERTY_SET --pvalue $PVALUE --impute-states $IMPUTE_STATES --output $RESULTS_FN > \"$PROGRESS_FILE\""
    srun --mpi=$MPI_TYPE -n $PROCS $HYPHY LIBPATH=$HYPHY_PATH $PRIME --alignment $FN --tree $TREE_FN --code $GENETIC_CODE --branches $BRANCHES --property-set $PROPERTY_SET --pvalue $PVALUE --impute-states $IMPUTE_STATES --output $RESULTS_FN > "$PROGRESS_FILE"
  fi
else
