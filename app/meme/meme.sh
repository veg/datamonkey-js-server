...
then
  ...
  if [ -n "$SLURM_JOB_ID" ]; then
    ...
    if [ -f "$HYPHY_MPI" ]; then
      ...
      echo "srun --mpi=$MPI_TYPE -n $PROCS $HYPHY_MPI LIBPATH=$HYPHY_PATH $MEME --alignment $FN --tree $TREE_FN --code $GENETIC_CODE --multiple-hits $MULTIPLE_HITS --site-multihit $SITE_MULTIHIT --rates $RATES --resample $RESAMPLE --impute-states $IMPUTE_STATES > \"$PROGRESS_FILE\""
      srun --mpi=$MPI_TYPE -n $PROCS $HYPHY_MPI LIBPATH=$HYPHY_PATH $MEME --alignment $FN --tree $TREE_FN --code $GENETIC_CODE --multiple-hits $MULTIPLE_HITS --site-multihit $SITE_MULTIHIT --rates $RATES --resample $RESAMPLE --impute-states $IMPUTE_STATES > "$PROGRESS_FILE"
    elif [ -f "$HYPHY_NON_MPI" ]; then
  ...
  if [ -n "$SLURM_JOB_ID" ]; then
    ...
    if [ -f "$HYPHY_MPI" ]; then
      echo "Using MPI HYPHY: $HYPHY_MPI"
      echo "srun --mpi=$MPI_TYPE -n $PROCS $HYPHY_MPI LIBPATH=$HYPHY_PATH $MEME --alignment $FN --tree $TREE_FN --code $GENETIC_CODE --multiple-hits $MULTIPLE_HITS --site-multihit $SITE_MULTIHIT --rates $RATES --impute-states $IMPUTE_STATES > \"$PROGRESS_FILE\""
      srun --mpi=$MPI_TYPE -n $PROCS $HYPHY_MPI LIBPATH=$HYPHY_PATH $MEME --alignment $FN --tree $TREE_FN --code $GENETIC_CODE --multiple-hits $MULTIPLE_HITS --site-multihit $SITE_MULTIHIT --rates $RATES --impute-states $IMPUTE_STATES > "$PROGRESS_FILE"
    elif [ -f "$HYPHY_NON_MPI" ]; then
      echo "HYPHYMPI not found, falling back to non-MPI HYPHY: $HYPHY_NON_MPI"
