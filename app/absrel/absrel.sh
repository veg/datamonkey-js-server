...
if [ -n "$SLURM_JOB_ID" ]; then
  ...
  if [ -f "$HYPHY_MPI" ]; then
    ...
    echo "srun --mpi=$MPI_TYPE -n $PROCS $HYPHY_MPI LIBPATH=$HYPHY_PATH $ABSREL --alignment $FN --tree $TREE_FN --code $GENETIC_CODE --branches $BRANCHES --multiple-hits $MULTIPLE_HITS --srv $SRV --blb $BLB --output $RESULTS_FN > \"$PROGRESS_FILE\""
    srun --mpi=$MPI_TYPE -n $PROCS $HYPHY_MPI LIBPATH=$HYPHY_PATH $ABSREL --alignment $FN --tree $TREE_FN --code $GENETIC_CODE --branches $BRANCHES --multiple-hits $MULTIPLE_HITS --srv $SRV --blb $BLB --output $RESULTS_FN > "$PROGRESS_FILE"
  elif [ -f "$HYPHY_NON_MPI" ]; then
