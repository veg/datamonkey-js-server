...
if [ -n "$SLURM_JOB_ID" ]; then
  ...
  if [ -f "$HYPHY_MPI" ]; then
    ...
    echo "srun --mpi=$MPI_TYPE -n $PROCS $HYPHY_MPI LIBPATH=$HYPHY_PATH $RELAX --code $GENETIC_CODE --alignment $FN --tree $TREE_FN --mode \"$MODE\" --test $TEST_BRANCHES --reference $REFERENCE_BRANCHES --models \"$MODELS\" --rates $OMEGA_RATE_CLASSES --kill-zero-lengths $KZERO --output $RESULT_FILE > \"$PROGRESS_FILE\""
    srun --mpi=$MPI_TYPE -n $PROCS $HYPHY_MPI LIBPATH=$HYPHY_PATH $RELAX --code $GENETIC_CODE --alignment $FN --tree $TREE_FN --mode "$MODE" --test $TEST_BRANCHES --reference $REFERENCE_BRANCHES --models "$MODELS" --rates $OMEGA_RATE_CLASSES --kill-zero-lengths $KZERO --output $RESULT_FILE > "$PROGRESS_FILE"
  elif [ -f "$HYPHY_NON_MPI" ]; then
