...
if [ -n "$SLURM_JOB_ID" ]; then
  ...
  if [ -f "$HYPHY_MPI" ]; then
    ...
    echo "srun --mpi=$MPI_TYPE -n $PROCS $HYPHY_MPI LIBPATH=$HYPHY_PATH $MULTIHIT --code $GENETIC_CODE --alignment $FN --tree $TREE_FN --rates $RATE_CLASSES --triple-islands $TRIPLE_ISLANDS --branches $BRANCHES --output $RESULTS_FN > \"$PROGRESS_FILE\""
    srun --mpi=$MPI_TYPE -n $PROCS $HYPHY_MPI LIBPATH=$HYPHY_PATH $MULTIHIT --code $GENETIC_CODE --alignment $FN --tree $TREE_FN --rates $RATE_CLASSES --triple-islands $TRIPLE_ISLANDS --branches $BRANCHES --output $RESULTS_FN > "$PROGRESS_FILE"
  elif [ -f "$HYPHY_NON_MPI" ]; then
