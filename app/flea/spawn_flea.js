...
FleaRunner.prototype.start = function(fn, flea_params) {
  ...
  var flea_pipeline_submit = function() {
    ...
    var flea_pipeline = spawn(self.python, flea_pipeline_parameters, {
      cwd: self.filedir
    });
    ...
    flea_pipeline.on("close", function(code) {
      ...
      // Save results files
      if (code === 0) {
        var frequency_promise = Q.nfcall(
          fs.readFile,
          self.frequencies_fn,
          "utf-8"
        );
        var rates_promise = Q.nfcall(fs.readFile, self.rates_fn, "utf-8");
        var rates_pheno_promise = Q.nfcall(
          fs.readFile,
          self.rates_pheno_fn,
          "utf-8"
        );
        var sequences_promise = Q.nfcall(
          fs.readFile,
          self.sequences_fn,
          "utf-8"
        );
        var trees_promise = Q.nfcall(fs.readFile, self.trees_fn, "utf-8");
        var divergence_promise = Q.nfcall(
          fs.readFile,
          self.divergence_fn,
          "utf-8"
        );
        var copynumbers_promise = Q.nfcall(
          fs.readFile,
          self.copynumbers_fn,
          "utf-8"
        );
        var coordinates_promise = Q.nfcall(
          fs.readFile,
          self.coordinates_fn,
          "utf-8"
        );
