...
if [ -n "$SLURM_JOB_ID" ]; then
  ...
  if [ -f "$HYPHY_MPI" ]; then
    ...
    echo "srun --mpi=$MPI_TYPE -n $PROCS $HYPHY_MPI LIBPATH=$HYPHY_PATH $BUSTED --code $GENETIC_CODE --alignment $FN --tree $TREE_FN --branches $BRANCHES --rates $omegaClasses --syn-rates $synRateClasses --multiple-hits $MULTIHIT --srv $SYN_RATE_VARIATION --grid-size $initialPointsInLikelihood --starting-points $initialGuesses --kill-zero-lengths $KZERO --error-sink $ERROR_SINK --output $RESULTS_FILE --save-fit $SAVE_FIT > \"$PROGRESS_FILE\""
    srun --mpi=$MPI_TYPE -n $PROCS $HYPHY_MPI LIBPATH=$HYPHY_PATH $BUSTED --code $GENETIC_CODE --alignment $FN --tree $TREE_FN --branches $BRANCHES --rates $omegaClasses --syn-rates $synRateClasses --multiple-hits $MULTIHIT --srv $SYN_RATE_VARIATION --grid-size $initialPointsInLikelihood --starting-points $initialGuesses --kill-zero-lengths $KZERO --error-sink $ERROR_SINK --output $RESULTS_FILE --save-fit $SAVE_FIT > "$PROGRESS_FILE"
  elif [ -f "$HYPHY_NON_MPI" ]; then
