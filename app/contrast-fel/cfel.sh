...
RATE_VARIATION="${rate_variation:-Yes}"
PERMUTATIONS="${permutations:-Yes}"
P_VALUE="${p_value:-0.05}"
Q_VALUE="${q_value:-0.20}"
PROCS=${procs:-1}
...
CONTRAST_FEL=$HYPHY_PATH/TemplateBatchFiles/SelectionAnalyses/contrast-fel.bf
...
if [ -n "$SLURM_JOB_ID" ]; then
  ...
  if [ -f "$HYPHY_MPI" ]; then
    ...
    echo "srun --mpi=$MPI_TYPE -n $PROCS $HYPHY_MPI LIBPATH=$HYPHY_PATH $CONTRAST_FEL --alignment $FN --tree $TREE_FN $BRANCH_SETS --srv $RATE_VARIATION --permutations $PERMUTATIONS --p-value $P_VALUE --q-value $Q_VALUE --output $RESULTS_FILE > \"$PROGRESS_FILE\""
    srun --mpi=$MPI_TYPE -n $PROCS $HYPHY_MPI LIBPATH=$HYPHY_PATH $CONTRAST_FEL --alignment $FN --tree $TREE_FN $BRANCH_SETS --srv $RATE_VARIATION --permutations $PERMUTATIONS --p-value $P_VALUE --q-value $Q_VALUE --output $RESULTS_FILE > "$PROGRESS_FILE"
  elif [ -f "$HYPHY_NON_MPI" ]; then
