...
if [ $DATATYPE == "nucleotide" ]; then
  ...
  $HYPHY LIBPATH=$HYPHY_PATH $BGM --branches "All" --code $GENETIC_CODE --type $DATATYPE --alignment $FN --tree $TREE_FN --steps $LENGTH --burn-in $BURNIN --samples $SAMPLES --max-parents $MAXIMUM_PARENTS --min-subs $MINIMUM_SUBSTITUTIONS --output $RESULTS_FILE >> $PROGRESS_FILE
  ...
  $HYPHY LIBPATH=$HYPHY_PATH $BGM --branches "All" --code $GENETIC_CODE --baseline_model $SUBSTITUTION_MODEL --type $DATATYPE --alignment $FN --tree $TREE_FN --steps $LENGTH --burn-in $BURNIN --samples $SAMPLES --max-parents $MAXIMUM_PARENTS --min-subs $MINIMUM_SUBSTITUTIONS --output $RESULTS_FILE >> $PROGRESS_FILE
