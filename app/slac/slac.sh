...
if [ -n "$SLURM_JOB_ID" ]; then
  ...
  export TOLERATE_NUMERICAL_ERRORS=1
  echo "mpirun -np $PROCS $HYPHY LIBPATH=$HYPHY_PATH $SLAC --code $GENETIC_CODE --alignment $FN --tree $TREE_FN --branches All --samples $NUM_SAMPLES --pvalue $PVAL --kill-zero-lengths $KZERO --output $RESULTS_FILE >> $PROGRESS_FILE"
  mpirun -np $PROCS $HYPHY LIBPATH=$HYPHY_PATH $SLAC --code $GENETIC_CODE --alignment $FN --tree $TREE_FN --branches All --samples $NUM_SAMPLES --pvalue $PVAL --kill-zero-lengths $KZERO --output $RESULTS_FILE >> $PROGRESS_FILE
fi
