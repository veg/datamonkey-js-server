...
if [ -n "$SLURM_JOB_ID" ]; then
  ...
  if [ -f "$HYPHY_MPI" ]; then
    ...
    echo "srun --mpi=$MPI_TYPE -n $PROCS $HYPHY_MPI LIBPATH=$HYPHY_PATH $SLAC --code $GENETIC_CODE --alignment $FN --tree $TREE_FN --branches $BRANCHES --samples $NUM_SAMPLES --pvalue $PVAL --kill-zero-lengths $KZERO --output $RESULTS_FILE > \"$PROGRESS_FILE\""
    srun --mpi=$MPI_TYPE -n $PROCS $HYPHY_MPI LIBPATH=$HYPHY_PATH $SLAC --code $GENETIC_CODE --alignment $FN --tree $TREE_FN --branches $BRANCHES --samples $NUM_SAMPLES --pvalue $PVAL --kill-zero-lengths $KZERO --output $RESULTS_FILE > "$PROGRESS_FILE"
  elif [ -f "$HYPHY_NON_MPI" ]; then
