...
// #453: cancelled is a terminal state — mark the scheduler hash and set the
// terminal retention TTL so it does not linger in Redis forever.
function markCancelled(torque_id) {
  client.hset(torque_id, "status", "cancelled", function(err) {
    if (err) logger.error(torque_id + " : redis hset failed: " + err.message);
    redisTtl.expireTerminal(client, torque_id);
  });
}
