  ...
  const checkWithSacct = () => {
    ...
    sacct.on("close", function(code) {
      ...
      try {
        ...
        // sacct reports cancelled jobs as "CANCELLED by <uid>" — keep only the
        // state token so it matches the valid_statuses map. Without this the
        // state falls through to "queued" and the watcher polls a cancelled
        // job forever, re-firing onJobCreated whose persistInFlight (#453)
        // strips the terminal TTL job.cancel just set.
        const job_status = items[2] ? items[2].trim().split(" ")[0] : '';
  ...
  const fallbackToSacct = function() {
    ...
    sacct.on("close", function(code) {
      ...
      try {
        ...
        // Normalize "CANCELLED by <uid>" to its state token (see returnJobStatus)
        const state_token = job_info.state ? String(job_info.state).trim().split(" ")[0] : "";
        if (state_token && state_token in valid_statuses) {
          job_info.status = valid_statuses[state_token];
        } else {
