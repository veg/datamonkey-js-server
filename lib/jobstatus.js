...
SlurmJobStatus.prototype.returnJobStatus = function(job_id, callback) {
  ...
  if (validateJobId(job_id)) {
    logger.info(`Job ID ${job_id} is valid, spawning sacct command`);
    const sacctParams = ["-j", self.job_id, "-o", "submit,start,state", "-P"];
    logger.info(`SLURM sacct command: sacct ${sacctParams.join(' ')}`);
    sacct = spawn("sacct", sacctParams);
  } else {
    self.error = job_id + " : invalid";
    logger.error(`Invalid job ID format: ${job_id}`);
    callback(self.error, "");
  ...
  sacct.stdout.on("data", function(data) {
    ...
    try {
      ...
      logger.info(`Raw sacct output: "${data}"`);
      
      // Parse sacct output which is pipe-delimited with a header row
      const lines = data.split('\n').filter(line => line.trim() !== '');
      
      if (lines.length < 2) {
        logger.warn(`Unexpected sacct output format - not enough lines: ${data}`);
        self.error = "Waiting for job to appear in SLURM accounting";
        callback(self.error, {
          status: "queued", // Default to queued while waiting
      ...
      const items = lines[lines.length - 1].split('|');
      ...
      // Only try to format dates if they're not empty
      if (job_ctime && job_ctime.trim() !== '') {
        try {
          job_ctime = moment(job_ctime).tz('America/Los_Angeles').tz('GMT').format();
        } catch (e) {
          logger.warn(`Error parsing creation time: ${job_ctime}`);
          job_ctime = new Date().toISOString();
        }
      } else {
        job_ctime = new Date().toISOString();
      }
      
      if (job_stime && job_stime.trim() !== '') {
        try {
      ...
      logger.warn(`Error parsing sacct output: ${e.message}`);
      logger.warn(`Stack trace: ${e.stack}`);
      logger.warn(`Raw data: "${data}"`);
      
      // Default to queued with basic info if we can't parse
      self.error = "Error parsing job status";
      callback(self.error, {
        status: "queued",
        ctime: new Date().toISOString(),
        stime: null
    ...
    if (job_status && status && job_status in status) {
      self.status = status[job_status];
      callback(self.error, {
        status: self.status,
        ctime: job_ctime,
...
logger.info(`Using job status class: ${config.submit_type === "qsub" ? "QsubJobStatus" : "SlurmJobStatus"}`);
