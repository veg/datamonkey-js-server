/**
 * Shared redis v6 (node-redis) client factory.
 *
 * v6 MIGRATION NOTES (2026-09, supersedes PR #472)
 * ------------------------------------------------
 *   - Node >= 20 required (engines + CI matrix updated; prod runs 22).
 *   - RESP3 is the default protocol (server must be redis >= 6.0; every
 *     instance we run is 6.2.x). None of the commands used here changed
 *     reply shape; hGetAll now returns plain objects instead of
 *     null-prototype objects, which is strictly more normal.
 *   - commandOptions.timeout now DEFAULTS to 5000ms — explicitly disabled
 *     in buildClientOptions(); see the comment there.
 *   - TRAP: variadic multi-pair hSet(key, f1, v1, f2, v2) SILENTLY writes
 *     only the first pair under v6 (no error). Use the object form
 *     hSet(key, { f1: v1, f2: v2 }) for multiple fields — every current
...
/**
 * Build the redis client options from config.json.
...
function buildClientOptions() {
  const options = {
    ...
    // redis@6 defaults commandOptions.timeout to 5000ms. Completed-job hashes
    // embed the FULL result JSON (read back on WS reconnect, no disk/Mongo
    // fallback), so a large hGetAll on a loaded server could exceed 5s and
    // start failing result delivery. Preserve the v5 no-timeout behaviour.
    commandOptions: {
      timeout: undefined,
    },
  };
