...
function spawnAnalysis(type, alignment, tree, analysisParams) {
  ...
  //
  // Tree placement matters — different analyses read the tree differently:
  //   - FEL/aBSREL/BUSTED/RELAX: read from params.analysis.tagged_nwk_tree || params.tree
  //   - MEME/PRIME: read from params.msa[0].nj, then check params.analysis.msa[0].usertree
  //   - SLAC: reads from params.msa[0].nj, then checks params.analysis.msa[0].usertree
  // So we set the tree in all locations to ensure every analysis finds it.
  //
  // hivtrace is special: it uses params._id directly.
  if (type === "hivtrace") {
    const params = Object.assign({ _id: jobId, submission_source: "mcp" }, analysisParams || {});
