...
function applyEnvOverrides(cfg) {
  ...
  // Redis memory-cleanup knobs (#453). TTLs for completed/terminal job keys and
  // the optional maxmemory best-effort CONFIG SET applied at client startup.
  if (process.env.DM_REDIS_RESULT_TTL)
    out.redis_result_ttl_seconds = parseInt(process.env.DM_REDIS_RESULT_TTL, 10);
  if (process.env.DM_REDIS_TERMINAL_TTL)
    out.redis_terminal_ttl_seconds = parseInt(process.env.DM_REDIS_TERMINAL_TTL, 10);
  if (process.env.DM_REDIS_MAXMEMORY) out.redis_maxmemory = process.env.DM_REDIS_MAXMEMORY;
  return out;
}
...
const schema = z
  .object({
    ...
    // Redis transient-data cleanup (#453). All optional — defaults are applied
    // at the redis-client helper (24h completed / 1h terminal) so an existing
    // config without these keys keeps working unchanged. redis_maxmemory is a
    // redis maxmemory string (e.g. "2gb") applied best-effort via CONFIG SET.
    redis_result_ttl_seconds: z.number().positive().optional(),
    redis_terminal_ttl_seconds: z.number().positive().optional(),
    redis_maxmemory: z.union([z.string(), z.number()]).optional()
  })
  .passthrough();
