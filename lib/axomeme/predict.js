...
const MODEL_VERSION = "v1-viral-finetuned";
 ...
 *
 * A DELIBERATE CHOICE, not a default, and it is the one place this port can disagree with the
 * browser about what a result MEANS. Three positions exist:
 *
 *   - The v1-viral reference pipeline has NO max_species and NO subsampling. It feeds all N taxa
 *     and expands the phylo tensors across the site batch.
 *   - datamonkey3 selects <= 512 by maximum phylogenetic diversity, because dist_matrix is N x N
 *     and a 25,000-sequence upload would be 2.5 GB in a browser tab.
 *   - This server matches datamonkey3 exactly.
 *
 * Matching is the point. The server has memory the browser does not and could feed all N — which
 * would track the reference more closely — but then the SAME alignment would score differently
 * depending on which execution mode the user picked, above 512 taxa, with nothing to indicate it.
 * That is precisely the failure this model swap was coordinated to avoid (veg/datamonkey3#209 is
 * the routing analogue; here it would be silent numeric disagreement). Cross-surface agreement
 * beats reference fidelity for a surrogate whose output is a RANKING within one alignment.
 *
 * If that trade is ever revisited, it has to be revisited on BOTH sides in the same release.
 */
const MIN_SPECIES = 2;
...
async function predictAxomeme(options = {}) {
  ...
  // dist_matrix and mds_coords are per-ALIGNMENT — batch() emits b memcpy'd copies
  ...
  // ONE head. v1-viral returns `lrt` alone; the 2.0 export also returned alpha / beta_neg /
  // beta_pos / p_neg, which fed the dS, dN+ and p columns. Those are gone with the model, and
  // vendor/postprocess.js omits the fields rather than emitting zeros for them.
