...
function prepareAlignment(input) {
	...
	//
	// This costs nothing to remove because nothing ever actually padded. `n` is the number of
	// SELECTED species, so every slot was already a real species and the mask was all zeros — it
	// existed only to satisfy the 2.0 graph's signature.
	return {
		speciesCount: n,
		totalCodons,
		windowSize,
