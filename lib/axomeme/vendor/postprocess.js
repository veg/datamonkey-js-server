 ...
 */
/**
 * postprocess.js — turning the graph's five raw output tensors into per-site results.
 *
 * THE RAW OUTPUTS ARE NOT THE NUMBERS A USER SHOULD SEE, and the transformations are not cosmetic:
 *
 *   - `alpha` / `beta_pos` are in LOG1P SPACE. The heads are `F.softplus(...)`, and
 *     expm1(softplus(x)) == exp(x), so the model predicts log1p(rate) and expm1 recovers the rate.
 *     Rendering them raw understates every rate — a dN of 1.7 shows as 1.0.
 *   - `p_neg` is a probability of NEGATIVE selection; the reported quantity is p_pos = 1 - p_neg.
 *   - `lrt` IS the LRT, already ordinal-decoded in-graph (the export is eval mode). It is NOT a log,
 *     which was worth checking rather than assuming: the reference derives
 *     `predicted_log_lrt = log1p(predicted_lrt)`, so the log column is the DERIVED one.
 *   - `beta_neg` is computed by the model and then never used by the reference's output. Carried here
 *     so it is available, but it appears in no column and no call.
 *
 ...
 *
 * @param {{lrt: ArrayLike<number>, alpha: ArrayLike<number>, beta_neg: ArrayLike<number>,
 *   beta_pos: ArrayLike<number>, p_neg: ArrayLike<number>}} outputs raw graph outputs, one per site
...
function buildPredictions(outputs, sites, callOptions = {}) {
	...
	for (let i = 0; i < n; i++) {
		...
		rows.push({
			...
			// expm1 undoes the softplus/log1p parameterisation; the clamp is the reference's.
			alphaDs: Math.max(0, Math.expm1(Number(outputs.alpha[i]))),
			betaPosDn: Math.max(0, Math.expm1(Number(outputs.beta_pos[i]))),
			pPos: 1 - Number(outputs.p_neg[i]),
			zScore: 0,
			percentile: 0,
			call: NEUTRAL_CALL
		});
