...
_local_GeneticCode = 0;
_local_scoreMatrix = "HIV_5";
_local_idx         = 6;
...
fscanf ( "accessoryMutationList.lst", "Lines", compMuList );
accMu = {};
accMu["PRIMARY_SITE"]		= {};
accMu["PRIMARY_WT"]			= {};
accMu["PRIMARY_RT"]			= {};
...
for ( _i = 1; _i < Columns ( compMuList ); _i += 1 ) {
    for (_j = _i+1; _j < Columns ( compMuList ); _j += 1) {
        primary		= compMuList[_i];
        secondary	= compMuList[_j];
        
        if (primary[1][Abs(primary)-2]  != secondary[1][Abs(secondary)-2]){
            
            ExecuteCommands ( "(accMu[\"PRIMARY_SITE\"])[\"" + _comp_count + "\"] = "  +  primary[1][Abs(primary)-2] + ";" );
            ExecuteCommands ( "(accMu[\"PRIMARY_WT\"])[\"" + _comp_count + "\"] = \"" + primary[0] + "\";" );
            ExecuteCommands ( "(accMu[\"PRIMARY_RT\"])[\"" + _comp_count + "\"] = \"" + primary[Abs(primary)-1] + "\";" );
            ExecuteCommands ( "(accMu[\"SECONDARY_SITE\"])[\"" + _comp_count + "\"] = " + secondary[1][Abs(secondary)-2] + ";" );
            ExecuteCommands ( "(accMu[\"SECONDARY_WT\"])[\"" + _comp_count + "\"] = \"" + secondary[0] + "\";" );
            ExecuteCommands ( "(accMu[\"SECONDARY_RT\"])[\"" + _comp_count + "\"] = \"" + secondary[Abs(secondary)-1] + "\";" );
            _comp_count += 1;
        }
    }
}

//fprintf (stdout, accMu, "\n");
	...
	if (  ( Abs ( span ) > 0  || Abs ( span2 ) > 0 ) ) {  /* good read */
		...
		*/
		...
		if ( perBase > SCORE_FACTOR*expectPerBase ) {
			for ( _m = 0; _m < Abs ( resultTables ); _m += 1 ) {
				...
				if ( ( site1 >= start_aa ) && ( site1 <= end_aa ) && ( site2 > start_aa ) && ( site2 <= end_aa ) && ( ( site1 != 0 ) && ( site2 != 0 ) ) ) {
					...
					//fprintf (stdout, wt1, rt1, wt2, rt2, "\n");
					
					mu1 = read_aa [ site1-start_aa-1 ];
					mu2 = read_aa [ site2-start_aa-1 ];
					if (  ( mu1 == rt1 ) && ( mu2 == rt2 ) ) { /*DR & ACC*/
						(resultTables[_m])[0][0] += 1;
					}
					if ( ( mu1 != rt1 ) && ( mu2 == rt2 ) ) { /*!DR & ACC*/
						(resultTables[_m])[0][1] += 1;
					} 
					if ( ( mu1 == rt1 ) && ( mu2 != rt2 ) ) { /*DR & !ACC*/
						(resultTables[_m])[1][0] += 1;
					}
					if ( ( mu1 != rt1 ) && ( mu2 != rt2 ) ) { /*!DR & !ACC*/
						(resultTables[_m])[1][1] += 1;
					}
...
fprintf (stdout, resultTables, "\n");
	
for ( _m = 0; _m < Abs ( resultTables ); _m = _m + 1 ) {
    testArray = resultTables[_m];
    fprintf (stdout, "Position\t", (accMu["PRIMARY_SITE"])[_m ], (accMu["PRIMARY_RT"])[_m ], "\t", (accMu["PRIMARY_SITE"])[_m ],(accMu["PRIMARY_WT"])[_m ], "\n",
                     (accMu["SECONDARY_SITE"])[_m ],(accMu["SECONDARY_RT"])[_m ], "\t", testArray[0][0], "\t", testArray[0][1], "\n",
                     (accMu["SECONDARY_SITE"])[_m ], (accMu["SECONDARY_WT"])[_m ], "\t", testArray[1][0], "\t", testArray[1][1], "\n",
                     "p-value = ", CChi2 (testArray, 0), "\n\n");
}
