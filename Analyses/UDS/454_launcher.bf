...
for ( _ak = 0; _ak < Rows ( _do_gene_map ); _ak = _ak + 1 ) {
	if ( _do_gene_map [_ak][0] ) {
		...
		ExecuteCommands ( "_options[\"02\"] = \"" + refOption + "\";" );
		ExecuteCommands ( "_options[\"03\"] = \"" + dagene + "\";" );
		if ( refOption != 2 ) {
			ExecuteCommands ( "_options[\"04\"] = \"" + dasequence + "\";" );
		}
		ExecuteCommands ( "_options[\"05\"] = \"" + scoreFileString + "\";" );
		ExecuteCommands ( "_options[\"06\"] = \"" + _in_minReadL + "\";" );
		ExecuteCommands ( "_options[\"07\"] = \"Y\";" ); /*automatic threshold selection */
...
fprintf ("usage.log",HTML_OUT[0][Abs(HTML_OUT)-2],",",raw454Sequences.species,",",raw454Sequences.sites,",",Time(1)-timer,",",_in_GeneticCodeTable,",",_in_scoreMatrix,",",_in_minReadL,"\n");
