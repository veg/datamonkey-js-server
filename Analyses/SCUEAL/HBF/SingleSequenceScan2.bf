...
//fprintf (stdout, HYPHY_LIB_DIRECTORY, "\n");
	...
	for (mpiNode=0; mpiNode < my_size; mpiNode = mpiNode+1)
	{
		...
		if (hasBannedBP[seq])
		{
			...
			*/
			...
			bannedSpan = (bannedSpan*Transpose(bpStencil))[0];
			if (bannedSpan > 0)
			{
				is_banned = 1;
				/*if (verboseFlag)
	...
	if (Rows(crf_group))
	{
		...
		if (haveGaps[0]>=0)
		{
			...
			for (k = Rows (haveGaps)-1; k > 0 ; k=k-2)
			{
				...
				if (haveGaps[k] != filteredData.sites - 1)
				{
			...
			if (verboseFlag)
			{
				fprintf (stdout, allSeqs[h], " has banned sites\n");
			}
			seq 							= nodeIDMap[seq+1];
...
for (k = 1; k <= crf_count; k=k+1)
{
	...
	if (h>0)
	{
		...
		(initialPops[h])[Abs(initialPops[h])] = encodeIndividual(individual%1);
	}
}
